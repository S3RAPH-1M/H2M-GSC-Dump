...
spawnDefensiveHarrier( lifeId, owner, pathStart, pathGoal )
{
	...
	harrier thread maps\mp\h2_killstreaks\_common::h2_sound_ent( "harrier_move" );
...
harrierTimer()
{
	...
	waitfor_duration_or_notify( 45 );
	self harrierLeave();
}

waitfor_duration_or_notify( duration )
{
	level endon( "game_ended" );
...
Callback_VehicleDamage( inflictor, attacker, damage, dFlags, meansOfDeath, weapon, point, dir, hitLoc, timeOffset, modelIndex, partName )
{
	...
	if ( self.health <= damage )
	{
		...
		{
			thread teamplayercardsplash( "destroyed_harrier_airstrike", attacker );
